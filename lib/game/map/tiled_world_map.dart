...
class CustomTiledMap extends TiledWorldMap {
  ...
  @Deprecated('Use "registerObj()" instead')
  @override
  void registerObject(String name, builder) {
    // DON'T USE THIS
  }

  void registerObj(String name, ObjectBuilder builder) {
    ...
    layer.objects.forEach(
      (element) {
        if (_objectsBuilder[element.name] != null) {
          ...
          Direction direction = type.getDirection(nullOk: true);
          AxisD axisD = type.getAxisD(nullOk: true);

          var object = _objectsBuilder[element.name](
            x,
            y,
            width,
            height,
  ...
  DataObjectCollision _getCollision(TileSet tileSetContain, int index) {
    ...
    if ((tileSetItemList?.isNotEmpty ?? false)) {
      ...
      List<CollisionArea> collisions = <CollisionArea>[];
